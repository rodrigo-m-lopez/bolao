# coding: utf-8
"""
Crawler / seeder para a Copa Sulamericana 2026.

Usa a API gratuita football-data.org (v4) para:
  1. seed_sulamericana(db)          — popula clubes e jogos da competição
  2. atualiza_resultados_sul(db)   — atualiza placares dos jogos encerrados

Variável de ambiente necessária:
  FOOTBALL_DATA_API_KEY — chave gratuita em https://www.football-data.org/client/register

Execução manual:
  python -m application.crawler_sulamericana seed    # popula times e jogos
  python -m application.crawler_sulamericana update  # atualiza resultados
"""
...
_COMPETITION = 'CSA'   # Copa Sulamericana no football-data.org
_SEASON = 2026
_TIMEOUT = 15
_COMPETICAO = 'Copa Sulamericana 2026'
...
def seed_sulamericana(db):
    """Popula o banco com clubes e jogos da Copa Sulamericana 2026.

    Idempotente: usa upsert, pode ser executado mais de uma vez sem duplicar dados.
    """
    logger.info('Iniciando seed da Copa Sulamericana 2026...')
    _seed_clubes(db)
    _seed_jogos(db)
    logger.info('Seed da Copa Sulamericana concluído.')
...
def _seed_jogos(db):
    logger.info('Buscando jogos da Sulamericana na API...')
...
def atualiza_resultados_sul(db):
    """Atualiza placares dos jogos encerrados e recalcula pontuações."""
    logger.info('Atualizando resultados da Copa Sulamericana...')
    data = _get(
    ...
    logger.info('Atualização de resultados da Copa Sulamericana concluída.')
...
if __name__ == '__main__':
    ...
    if cmd == 'seed':
        seed_sulamericana(_db)
    elif cmd == 'update':
        atualiza_resultados_sul(_db)
    else:
        print('Uso: python -m application.crawler_sulamericana [seed|update]')
        sys.exit(1)
